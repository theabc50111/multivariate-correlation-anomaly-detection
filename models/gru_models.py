...
class GRUCorrCoefPred(torch.nn.Module):
    def __init__(self, model_cfg: dict, **unused_kwargs):
        super(GRUCorrCoefPred, self).__init__()
        # set model config
    ...
    def init_best_model_info(self, train_data: dict, loss_fns: dict, epochs: int):
        ...
        self.best_model_info = {"num_training_data": train_data['model_input'].shape[1],
                                ...
                                "input_data_cus_disc_bins": self.model_cfg['input_data_cus_disc_bins'],
    ...
    def init_epoch_metrics(self, loss_fns: dict):
        ...
        self.epoch_metrics.update({(sp+"_"+str(fn)): torch.zeros(1) for sp, fn in product(data_split, loss_fns["fns"])})
    ...
    def record_epoch_metrics_each_batch(self, batch_loss: torch.Tensor, batch_loss_each_loss_fn: dict, batch_edge_acc: torch.Tensor, num_batches: int, rec_stage: str):
        ...
        if rec_stage == "train":
            ...
            epoch_metrics["fc_dec_gradient"] += sum([p.grad.sum() for p in self.fc_decoder.parameters() if p.grad is not None])/num_batches
        elif rec_stage == "val":
    ...
    def ret_best_model(self, epoch_i: int):
        ...
        epoch_metrics = self.epoch_metrics
        best_model = None
        if best_model_info.get("max_val_edge_acc"):
        ...
        elif best_model_info.get("min_val_loss"):
...
class GRUCorrClass(GRUCorrCoefPred):
    def __init__(self, model_cfg: dict, **unused_kwargs):
        super(GRUCorrClass, self).__init__(model_cfg)
        # set model config
        self.model_cfg = model_cfg
        ...
        self.class_fc_module_list = torch.nn.ModuleList()
        self.class_fc = Sequential(Linear(self.fc_dec_out_dim, self.class_fc_out_dim), Dropout(self.model_cfg["drop_p"] if "class_fc" in self.model_cfg["drop_pos"] else 0))
        for class_i in range(self.num_labels_classes):
            self.class_fc_module_list.add_module('class_{class_i}', self.class_fc)
        self.softmax = Softmax(dim=0)
        self.init_optimizer()

    def forward(self, x, *unused_args, **unused_kwargs):
        batch_pred_probs = torch.empty(x.shape[0], self.num_labels_classes, self.class_fc_out_dim)
        gru_output, gru_hn = self.gru(x)
        for data_batch_idx in range(x.shape[0]):
            ...
            for class_i, class_fc in enumerate(self.class_fc_module_list):
    ...
    def init_best_model_info(self, train_data: dict, loss_fns: dict, epochs: int):
        ...
        del self.best_model_info["min_val_loss"]
        self.best_model_info.update({"max_val_edge_acc": 0,
    ...
    def init_epoch_metrics(self, loss_fns: dict):
        ...
        self.epoch_metrics.update({"class_fc_gradient": torch.zeros(1)})
    ...
    def record_epoch_metrics_each_batch(self, batch_loss: torch.Tensor, batch_loss_each_loss_fn, batch_edge_acc: torch.Tensor, num_batches: int, rec_stage: str):
        super().record_epoch_metrics_each_batch(batch_loss, batch_loss_each_loss_fn, batch_edge_acc, num_batches, rec_stage)
        if rec_stage == "train":
            self.epoch_metrics["class_fc_gradient"] += sum([p.grad.sum() for p in self.class_fc_module_list.parameters() if p.grad is not None])/num_batches
    ...
    def train(self, mode: bool = True, train_data: np.ndarray = None, val_data: np.ndarray = None, loss_fns: dict = None, epochs: int = 1000, **unused_kwargs):
        ...
        for epoch_i in tqdm(range(epochs)):
            ...
            _, _, val_preds, val_labels = self.test(val_data, loss_fns=loss_fns, test_data_split="val")
            # record training history
            last_batch_output = {'tr_batch_preds': batch_preds, 'tr_labels': batch_y_labels, 'val_preds': val_preds, 'val_labels': val_labels}
            self.record_history(last_batch_output, epoch_i)
            # get best model
            ret_model = self.ret_best_model(epoch_i)
            if ret_model is not None:
